        ...
        void ReportCompletion()
        {
            ...
            try
            {
                while (true)
                {
                    ...
                    // Coalesce completions behind a slow callback. One worker reports in
                    // order while its siblings immediately refill their STAT slots.
                    progress.Report(next);
                }
            }
            catch
            {
                lock (progressLock) reporting = false;
                throw;
            }
        }
    ...
    )
    {
        foreach (var segmentId in segmentIds)
        {
            ...
            yield return new PipelinedStatResult
            {
                ...
                DefinitivelyMissing = UsenetArticleAvailability.IsDefinitiveMissing(response),
            };
        }
    }
