...
public class ConnectionPoolStats
{
    ...
    /// <summary>
    /// Aggregates the split transfer/metadata picture across <see cref="ProviderType.Pooled"/>
    /// providers only, matching the scope of the totals this summary is appended to
    /// (<c>_totalLive</c>/<c>_totalMax</c>/<c>_totalIdle</c>) and the widget's
    /// "on pooled providers" label. Counting backups here would let the transfer count
    /// exceed the pooled ceiling rendered beside it.
    ///
    /// Note the deliberate asymmetry with <c>_splitSummaryEnabled</c>, which requires every
    /// non-disabled provider to declare a transfer limit: a backup provider decides whether
    /// the split view is shown at all, but never contributes counts to it.
    /// </summary>
