...
public class MultiConnectionConnectionBudgetTests
{
    ...
    [Fact]
    public async Task ExactProviderLease_SkipsOnlyItsOwnMetadataAdmission()
    {
        var config = new ConfigManager();
        config.UpdateValues([
            new ConfigItem
        ...
        using var gate = new HealthCheckConnectionGate(config);
        using var globalLease = await gate.AcquireAsync(
            HealthCheckAdmissionPriority.Background, CancellationToken.None);
        var firstState = new BlockingStatState();
        var secondState = new BlockingStatState();
        var first = CreateClient(firstState, maxTransferConnections: 4, providerKey: "first");
        var second = CreateClient(secondState, maxTransferConnections: 4, providerKey: "second");
        using var providers = new MultiProviderNntpClient([first, second]);
        using var registry = new HealthCheckProviderAdmissionRegistry();
        registry.Activate(providers.HealthAdmissionGeneration);
        using var providerLease = Assert.IsType<HealthCheckProviderLease>(
            registry.TryAcquireMetadata("first", SemaphorePriority.Low).Lease);
        using var cts = new CancellationTokenSource();
        using var healthContext = cts.Token.SetContext<HealthCheckAdmissionContext>(
        ...
        var firstRequest = first.StatAsync("first@example", cts.Token);
        await WaitForEnteredCount(firstState, expected: 1);

        Assert.Equal(1, first.GetConnectionAdmissionSnapshot()?.ActiveMetadataOperations);

        var secondRequest = second.StatAsync("second@example", cts.Token);
        await WaitForEnteredCount(secondState, expected: 1);
        Assert.Equal(1, second.GetConnectionAdmissionSnapshot()?.ActiveMetadataOperations);

        firstState.ReleaseAll();
        secondState.ReleaseAll();
        await Task.WhenAll(firstRequest, secondRequest).WaitAsync(TestTimeout);
    }

    [Fact]
    public async Task StaleGenerationLease_DoesNotSkipReplacementProviderAdmission()
    {
        var config = new ConfigManager();
        config.UpdateValues([
            new ConfigItem
        ...
        using var globalLease = await gate.AcquireAsync(
            HealthCheckAdmissionPriority.Background, CancellationToken.None);
        var oldState = new BlockingStatState();
        var newState = new BlockingStatState();
        var oldProvider = CreateClient(oldState, 4, providerKey: "provider-a");
        var newProvider = CreateClient(newState, 4, providerKey: "provider-a");
        using var oldGeneration = new MultiProviderNntpClient([oldProvider]);
        using var newGeneration = new MultiProviderNntpClient([newProvider]);
        using var registry = new HealthCheckProviderAdmissionRegistry();
        ...
        var request = newProvider.StatAsync("replacement@example", cts.Token);
        await WaitForEnteredCount(newState, expected: 1);

        Assert.Equal(1, oldProvider.GetConnectionAdmissionSnapshot()?.ActiveMetadataOperations);
        Assert.Equal(1, newProvider.GetConnectionAdmissionSnapshot()?.ActiveMetadataOperations);
    ...
    [Fact]
    public async Task SaturatedProvider_DoesNotHoldGlobalHealthAdmissionWhileWaitingLocally()
    {
        var config = new ConfigManager();
        config.UpdateValues([
            new ConfigItem
        ...
        using var gate = new HealthCheckConnectionGate(config);
        var saturatedState = new BlockingStatState();
        var readyState = new BlockingStatState();
        using var saturatedProvider = CreateClient(saturatedState, maxTransferConnections: 4);
        using var readyProvider = CreateClient(readyState, maxTransferConnections: 4);

        var saturatingRequests = StartStats(saturatedProvider, count: 2);
        await WaitForEnteredCount(saturatedState, expected: 2);

        using var cts = new CancellationTokenSource();
        using var healthContext = cts.Token.SetContext(
            new HealthCheckAdmissionContext(gate, HealthCheckAdmissionPriority.Background));
        var locallyBlocked = saturatedProvider.StatAsync("blocked@example", cts.Token);
        var ready = readyProvider.StatAsync("ready@example", cts.Token);

        await WaitForEnteredCount(readyState, expected: 1);
        Assert.Equal(1, gate.GetSnapshot().Active);
        Assert.False(locallyBlocked.IsCompleted);

        readyState.ReleaseAll();
        await ready.WaitAsync(TestTimeout);
        saturatedState.ReleaseAll();
        await Task.WhenAll(saturatingRequests.Append(locallyBlocked)).WaitAsync(TestTimeout);
        Assert.Equal(0, gate.GetSnapshot().Active);
    }
    ...
    private static MultiConnectionNntpClient CreateClient(
        ...
        string providerKey = "budget-test")
    {
