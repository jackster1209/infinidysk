...
{
    [Fact]
    public async Task TryAcquireMetadata_WhenSaturated_DoesNotCreateAWaiter()
    {
        using var admission = CreateAdmission(providerLimit: 1, transferLimit: 1);
        using var held = await AcquireMetadata(admission);

        var acquired = admission.TryAcquire(
        ...
        Assert.Equal(1, snapshot.ActiveMetadataOperations);
        Assert.Equal(0, snapshot.WaitingMetadataOperations);
    }

    [Fact]
    public async Task TryAcquireMetadata_DoesNotBypassAnExistingMetadataWaiter()
    {
        var providerLimit = 1;
        ...
        Volatile.Write(ref providerLimit, 2);
        var acquired = admission.TryAcquire(
            ProviderConnectionKind.Metadata,
            SemaphorePriority.Low,
            out var lease);
        ...
        Assert.Null(lease);
        transfer.Dispose();
        using var metadata = await waitingMetadata.WaitAsync(TestTimeout);
    }

    [Fact]
    public async Task TryAcquireMetadata_DoesNotBypassAWaitingTransferThatCanRun()
    {
        var providerLimit = 1;
        using var admission = new ProviderConnectionAdmission(
            () => Volatile.Read(ref providerLimit),
    ...
    [Fact]
    public async Task EqualTransferLimit_AdmitsWaitingMetadataBeforeRefillingTransfers()
    {
        using var admission = CreateAdmission(providerLimit: 4, transferLimit: 4);
        var transfers = new List<ProviderConnectionAdmission.Lease>();
        try
        {
            for (var i = 0; i < 4; i++)
                transfers.Add(await AcquireTransfer(admission));
            ...
            metadata.Dispose();
            using var replacementTransfer = await waitingTransfer.WaitAsync(TestTimeout);
        }
        finally
        {
    ...
    [Fact]
    public async Task CancellationWonDuringEnqueue_DoesNotNotifyWhileAdmissionLockIsHeld()
    {
        using var cancellation = new CancellationTokenSource();
        var cancelWhenLimitIsRead = false;
        var observeNotifications = false;
        var notificationCouldReadSnapshot = false;
        ProviderConnectionAdmission? admission = null;
        admission = new ProviderConnectionAdmission(
            () =>
            {
                if (cancelWhenLimitIsRead)
                    cancellation.Cancel();
                return 1;
            },
            configuredTransferLimit: 1,
            onChanged: _ =>
            {
                ...
                notificationCouldReadSnapshot = Task.Run(() => admission!.GetSnapshot())
                    .Wait(TestTimeout);
            });
        using (admission)
        using (await AcquireTransfer(admission))
        {
            observeNotifications = true;
            cancelWhenLimitIsRead = true;
            ...
            await Assert.ThrowsAnyAsync<OperationCanceledException>(() => canceledWaiter);
            Assert.True(notificationCouldReadSnapshot);
        }
    }
